...
/**
 * Layer implementation for Objekte unter Vertrag based on OL VectorImageLayer
 */
export default class ObjekteUnterVertragLayer extends FeatureLayer {

  private readonly featureStyle: FilialeFeatureStyle = new FilialeFeatureStyle(layersConfig[LayerIdentifier.OBJEKTE_UNTER_VERTRAG].color, layersConfig[LayerIdentifier.OBJEKTE_UNTER_VERTRAG].text_color);

  constructor(http: HttpClient, private layerManagerFacade: LayerManagerFacade) {
    super(http);
    this.createLayer();
    this.layerManagerFacade.getLayerConfig$(LayerIdentifier.OBJEKTE_UNTER_VERTRAG).subscribe((config) => {
  ...
  private createLayer(): void {
    ...
    const source = this.createVectorSource(environment.layer_features_endpoint_base + LayerIdentifier.OBJEKTE_UNTER_VERTRAG, grid);
    layer.setSource(source);

    layer.setMinZoom(layersConfig[LayerIdentifier.OBJEKTE_UNTER_VERTRAG].minZoom);
    layer.setStyle(this.featureStyle.getStyleFunction());
    layer.set(FeatureLayer.LAYER_NAME_PROPERTY, LayerIdentifier.OBJEKTE_UNTER_VERTRAG);
    this._layer = layer;
  }
}
