...
export default class FilialenGeschlossenLayer extends FeatureLayer {

  private readonly featureStyle: FilialeFeatureStyle =
    new FilialeFeatureStyle(layersConfig[LayerIdentifier.FILIALEN_GESCHLOSSEN].color, layersConfig[LayerIdentifier.FILIALEN_GESCHLOSSEN].text_color);

  constructor(http: HttpClient, private layerManagerFacade: LayerManagerFacade) {
    super(http);
    this.createLayer();
    this.layerManagerFacade.getLayerConfig$(LayerIdentifier.FILIALEN_GESCHLOSSEN).subscribe((config) => {
  ...
  private createLayer(): void {
    ...
    const source = this.createVectorSource(environment.layer_features_endpoint_base + LayerIdentifier.FILIALEN_GESCHLOSSEN, grid);
    layer.setSource(source);

    layer.setMinZoom(layersConfig[LayerIdentifier.FILIALEN_GESCHLOSSEN].minZoom);
    layer.setStyle(this.featureStyle.getStyleFunction());
    layer.set(FeatureLayer.LAYER_NAME_PROPERTY, LayerIdentifier.FILIALEN_GESCHLOSSEN);
    this._layer = layer;
  }
}
