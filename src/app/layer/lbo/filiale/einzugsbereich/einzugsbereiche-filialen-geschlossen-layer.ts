...
export default class EinzugsbereicheFilialenGeschlossenLayer extends FeatureLayer {

  private readonly ezbFeatureLayerStyle: EinzugsbereicheFeatureStyle = new EinzugsbereicheFeatureStyle(layersConfig[LayerIdentifier.EINZUGSBEREICHE_FILIALEN_GESCHLOSSEN].fill_color);

  constructor(http: HttpClient, private layerManagerFacade: LayerManagerFacade) {
    super(http);
    this.createLayer()
    this.layerManagerFacade.getLayerConfig$(LayerIdentifier.EINZUGSBEREICHE_FILIALEN_GESCHLOSSEN).subscribe((config) => {
  ...
  private createLayer(): void {
    ...
    const source = this.createVectorTileSource(environment.layer_features_endpoint_base + LayerIdentifier.EINZUGSBEREICHE_FILIALEN_GESCHLOSSEN,
                                               {maxZoom: 13}); // do not load smaller tiles to prevent many requests, assuming there is no more detail
    layer.setSource(source);

    layer.setMinZoom(layersConfig[LayerIdentifier.EINZUGSBEREICHE_FILIALEN_GESCHLOSSEN].minZoom); // approx. resolution < 50
    layer.setStyle(this.ezbFeatureLayerStyle.getStyleFunction());
    layer.set(FeatureLayer.LAYER_NAME_PROPERTY, LayerIdentifier.EINZUGSBEREICHE_FILIALEN_GESCHLOSSEN);
    this._layer = layer;
  }
}
