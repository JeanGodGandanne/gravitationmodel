...
export default class WettbewerberOffenLayer extends FeatureLayer {

  private readonly featureStyle: WettbewerberFeatureStyle =
    new WettbewerberFeatureStyle(layersConfig[LayerIdentifier.WETTBEWERBER_OFFEN].color, layersConfig[LayerIdentifier.WETTBEWERBER_OFFEN].text_color);

  constructor(http: HttpClient, private layerManagerFacade: LayerManagerFacade) {
    super(http);
    this.createLayer();
    this.layerManagerFacade.getLayerConfig$(LayerIdentifier.WETTBEWERBER_OFFEN).subscribe((config) => {
  ...
  private createLayer(): void {
    ...
    const source = this.createVectorSource(environment.layer_features_endpoint_base + LayerIdentifier.WETTBEWERBER_OFFEN, grid);
    layer.setSource(source);

    layer.setMinZoom(layersConfig[LayerIdentifier.WETTBEWERBER_OFFEN].minZoom);
    layer.setStyle(this.featureStyle.getStyleFunction());
    layer.set(FeatureLayer.LAYER_NAME_PROPERTY, LayerIdentifier.WETTBEWERBER_OFFEN);
    this._layer = layer;
  }
}
