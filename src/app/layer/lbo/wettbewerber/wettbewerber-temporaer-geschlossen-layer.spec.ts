...
import WettbewerberTemporaerGeschlossenLayer from './wettbewerber-temporaer-geschlossen-layer';
...
describe('WettbewerberTemporaerGeschlossenLayer', () => {
  let testee: WettbewerberTemporaerGeschlossenLayer;
  const http = mock(HttpClient);
  const layerManagerFacade = mock(LayerManagerFacade);
  const wettbewerberTemporaerGeschlossenLayerConfig$ = new Subject<LayerConfigType>();

  beforeEach(() => {
    reset(http);
    when(http.get(anyString())).thenReturn(of(TestDataConstants.TEST_POINT_GEOJSON));
    when(layerManagerFacade.getLayerConfig$(LayerIdentifier.WETTBEWERBER_TEMPORAER_GESCHLOSSEN)).thenReturn(wettbewerberTemporaerGeschlossenLayerConfig$);
    testee = new WettbewerberTemporaerGeschlossenLayer(instance(http), instance(layerManagerFacade));
  });
  ...
  test('should set name', () => {
    expect(testee.layer.get('name')).toBe('wett_tmpg');
  });
