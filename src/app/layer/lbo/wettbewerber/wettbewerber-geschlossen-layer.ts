...
export default class WettbewerberGeschlossenLayer extends FeatureLayer {

  private readonly featureStyle: WettbewerberFeatureStyle =
    new WettbewerberFeatureStyle(layersConfig[LayerIdentifier.WETTBEWERBER_GESCHLOSSEN].color, layersConfig[LayerIdentifier.WETTBEWERBER_GESCHLOSSEN].text_color);

  constructor(http: HttpClient, private layerManagerFacade: LayerManagerFacade) {
    super(http);
    this.createLayer();
    this.layerManagerFacade.getLayerConfig$(LayerIdentifier.WETTBEWERBER_GESCHLOSSEN).subscribe((config) => {
  ...
  private createLayer(): void {
    ...
    const source = this.createVectorSource(environment.layer_features_endpoint_base + LayerIdentifier.WETTBEWERBER_GESCHLOSSEN, grid);
    layer.setSource(source);

    layer.setMinZoom(layersConfig[LayerIdentifier.WETTBEWERBER_GESCHLOSSEN].minZoom);
    layer.setStyle(this.featureStyle.getStyleFunction());
    layer.set(FeatureLayer.LAYER_NAME_PROPERTY, LayerIdentifier.WETTBEWERBER_GESCHLOSSEN);
    this._layer = layer;
  }
}
