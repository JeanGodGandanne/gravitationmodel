...
import WettbewerberImBauLayer from './wettbewerber-im-bau-layer';
...
describe('WettbewerberImBauLayer', () => {
  let testee: WettbewerberImBauLayer;
  const http = mock(HttpClient);
  const layerManagerFacade = mock(LayerManagerFacade);
  const wettbewerberImBauLayerConfig$ = new Subject<LayerConfigType>();

  beforeEach(() => {
    reset(http);
    when(http.get(anyString())).thenReturn(of(TestDataConstants.TEST_POINT_GEOJSON));
    when(layerManagerFacade.getLayerConfig$(LayerIdentifier.WETTBEWERBER_IM_BAU)).thenReturn(wettbewerberImBauLayerConfig$);
    testee = new WettbewerberImBauLayer(instance(http), instance(layerManagerFacade));
  });
  ...
  test('should set name', () => {
    expect(testee.layer.get('name')).toBe('wett_bau');
  });
