...
import WettbewerberGeschlossenLayer from './wettbewerber-geschlossen-layer';
...
describe('WettbewerberGeschlossenLayer', () => {
  let testee: WettbewerberGeschlossenLayer;
  const http = mock(HttpClient);
  const layerManagerFacade = mock(LayerManagerFacade);
  const wettbewerberGeschlossenLayerConfig$ = new Subject<LayerConfigType>();

  beforeEach(() => {
    reset(http);
    when(http.get(anyString())).thenReturn(of(TestDataConstants.TEST_POINT_GEOJSON));
    when(layerManagerFacade.getLayerConfig$(LayerIdentifier.WETTBEWERBER_GESCHLOSSEN)).thenReturn(wettbewerberGeschlossenLayerConfig$);
    testee = new WettbewerberGeschlossenLayer(instance(http), instance(layerManagerFacade));
  });
  ...
  test('should set name', () => {
    expect(testee.layer.get('name')).toBe('wett_gesch');
  });
